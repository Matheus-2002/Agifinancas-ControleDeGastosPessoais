...
public class UsuarioDAO {
    ...
    public void criarUsuario(Usuario usuario) {
        String criarUsuario = "INSERT INTO usuario (name, email, senha) VALUES (?, ?, ?)";
        try (PreparedStatement stmt = connection.prepareStatement(criarUsuario)){
    ...
    public Usuario verificarUsuario (String email, String senha) {
        String verificarUsuario = "SELECT id, name, email, senha FROM usuario WHERE email = ?";
        try (Connection conn = JDBC_Connection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(verificarUsuario)) {
            stmt.setString(1, email);
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    int id = rs.getInt("id");
                    String nome = rs.getString("name");
                    String emailArmazenado = rs.getString("email");
                    String senhaArmazenada = rs.getString("senha");
                    if(BCrypt.checkpw(senha, senhaArmazenada)){
                        return new Usuario(nome, emailArmazenado, id);
                    }else{
                        MensagemAlerta.mostrarAlerta("Senha invalida", "Sua senha esta errada digite a senha certa ou troque sua senha", Alert.AlertType.ERROR);
                    }
                }else {
                    MensagemAlerta.mostrarAlerta("Email Invalido", "Seu email esta errado digite um email certo", Alert.AlertType.ERROR);
                }
            }
        } catch (SQLException e) {
            MensagemAlerta.mostrarAlerta("Sql", "Erro na pesquisa do banco de dados usuarioDAO.verificarUsuario", Alert.AlertType.ERROR);
        }
        return null;
    }
